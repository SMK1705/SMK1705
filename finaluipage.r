...
ui <- navbarPage("Stock Analysis",
                 tabPanel("Predict",fluidPage(
                   ...
                   actionButton("show", "Show Overview"),
                   tableOutput("disp"),
                   verbatimTextOutput("summary"),
                   sidebarLayout(sidebarPanel(radioButtons('plots','Plots:',choices = c('Time Series','Auto Correlation','Forecast'))),
...
server <- function(input, output, session) {
  dataraw <- reactive({read.csv(paste("D:\\SMK\\DA\\R\\Project\\Datasets\\",input$comName,".csv",sep=""))})
  data<-reactive({
    data1=dataraw()
